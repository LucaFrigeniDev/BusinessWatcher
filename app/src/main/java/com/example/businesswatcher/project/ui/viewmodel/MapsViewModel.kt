...
@Inject constructor(
    ...
    fun getGroupName(groupId: Long?, companyName: String): MutableLiveData<String> {
        val title: MutableLiveData<String> = MutableLiveData(companyName)
        if (groupId != null) {
            groupRepository.getGroup(groupId).onEach {
                title.postValue(it.name)
            }.launchIn(viewModelScope)
        }
        return title
    }
